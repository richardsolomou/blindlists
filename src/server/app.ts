...
const globalApp = globalThis as typeof globalThis & { __blindlists?: App }

export function app(): App {
  if (!globalApp.__blindlists) {
    const database = openDatabase(databasePath())
    const service = new BlindListsService(new Repository(database))
    globalApp.__blindlists = { database, service }
