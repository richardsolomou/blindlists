...
- **Identity is the session, membership is the authorization.** better-auth (`src/server/auth.ts`) owns accounts; `requireUser()` in `src/server/session.ts` gets the signed-in user, and `Service.requireMembership` decides what they may touch. The crew link is an invitation: it grants seeing the crew's name, its members, and the right to join — nothing more. `crewView` deliberately returns `currentGame: null` and empty `pastGames` to a link holder who has not joined, so a leaked link never exposes a list.
...
- **`entries.user_id` points at the account, not at crew membership.** That is what lets someone leave a crew — a plain delete of their `crew_members` row — without erasing their lists from games that already revealed. Never re-key entries to membership.
