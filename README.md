...
- `src/server/presence.ts` — who has a group page open and who is typing, held for as long as their stream is.
- `src/start.ts`, `src/server/canonicalHost.ts` — the global request middleware that keeps one canonical hostname.
